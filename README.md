...
本项目仅供学习与技术参考，是设计评审与可运行原型，不构成生产部署方案。运行过程中会创建 AWS 资源并调用 Amazon Bedrock 模型，产生费用，请在实验结束后及时清理。作者不对因使用本项目产生的任何费用或损失承担责任。本项目与 Amazon Web Services 及 Moonshot AI 无官方关联，相关服务的可用性与定价以各方官方文档为准。PNR/GDS 指令、航司规则等为脱敏示例，不代表真实业务规范；生产环境使用前请根据实际需求进行安全评估与调整。
